import Head from 'next/head';
import styles from '../styles/equip.module.css';
import Paragraph from '@/components/Paragraph';

const Equip = () => {
  return (
    <>
      <Head>
        <title>Equip His Army</title>
        <meta
          name='description'
          content='Ephesians 4:12 called. Operation 412 answered: equip believers for the work of ministry; build up the Body of Christ.'
        />
      </Head>

      <main className={styles.container}>
        <h1 className={styles.title}>EQUIP HIS ARMY</h1>
        <Paragraph variant='cursiveCentered'>EPHESIANS 4:12 = OPERATION 412</Paragraph>
        <Paragraph variant='center'>Ephesians 4:12 called. Operation 412 answered:</Paragraph>
        <Paragraph variant='left'>Equip believers for the work of ministry.</Paragraph>
      </main>
    </>
  );
};

export default Equip;
